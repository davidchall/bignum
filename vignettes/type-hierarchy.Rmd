...
* `biginteger` expands the set of integer values supported by `double` (in fact, it supports _any_ integer value). But it _does not_ support non-integer values.
* `bigfloat` expands the set of values supported by `double` (both in precision and range), but _does not_ support the entire range of integers supported by `biginteger`.
